...
def save_lidar_points(frame, cur_save_path, ri_index=0):
    range_images, camera_projections, range_image_top_pose = \
        frame_utils.parse_range_image_and_camera_projection(frame)

    points, cp_points, points_in_NLZ_flag, points_intensity, points_elongation = \
        convert_range_image_to_point_cloud(frame, range_images, camera_projections, range_image_top_pose, ri_index=ri_index)
...
def process_single_sequence(sequence_file, save_path, sampled_interval, has_label=True, dataset_cfg=None, both_returns=False, test_set=False):
    ...
    if both_returns:
        if test_set:
            second_return_tag = dataset_cfg.get('SECOND_RETURN_PROCESSED_DATA_TAG','waymo_processed_data_second_return_test_set')
        else:
            second_return_tag = dataset_cfg.get('SECOND_RETURN_PROCESSED_DATA_TAG','waymo_processed_data_second_return')
        cur_save_dir_2nd_ret = save_path.parent / second_return_tag / sequence_name
        cur_save_dir_2nd_ret.mkdir(parents=True, exist_ok=True)
        ...
        if both_returns:
            for cnt, data in enumerate(dataset):
                if cnt % sampled_interval != 0:
                    continue
                npy_file_2nd_ret = cur_save_dir_2nd_ret / ('%04d.npy' % cnt)
                if not npy_file_2nd_ret.exists():
                    frame = dataset_pb2.Frame()
                    frame.ParseFromString(bytearray(data.numpy()))
                    _ = save_lidar_points(frame, npy_file_2nd_ret, ri_index=1)
                else:
                    print('BOTH_RETURNS: Skip sequence since it has been processed before: %s' % npy_file_2nd_ret)
    ...
    for cnt, data in enumerate(dataset):
        ...
        if both_returns:
            cur_save_dir_2nd_ret = save_path.parent / dataset_cfg.SECOND_RETURN_PROCESSED_DATA_TAG / sequence_name
            cur_save_dir_2nd_ret.mkdir(parents=True, exist_ok=True)
            _ = save_lidar_points(frame, cur_save_dir_2nd_ret / ('%04d.npy' % cnt), ri_index=1)
...
def extract_densification_infos(sequence_file, sampled_interval, max_sweeps=0):
    sequence_name = os.path.splitext(os.path.basename(sequence_file))[0]
    ...
    for cnt, data in enumerate(dataset):
        ...
        frame.ParseFromString(bytearray(data.numpy()))
        info = {}
        info['lidar_sequence'] = sequence_name
        info['sample_idx'] = cnt
        curr_time_micro = frame.timestamp_micros
        curr_veh_pose = np.array(frame.pose.transform, dtype=np.float32).reshape(4, 4)
        global_to_curr_veh_frame = np.linalg.inv(curr_veh_pose)
        ...
        while len(sweeps) < max_sweeps - 1:
            if cnt <= 0:
                ...
                cnt -= 1
                unsaved_sweeps -= 1
                prev_frame = dataset_pb2.Frame()
                prev_frame.ParseFromString(bytearray(sliding_window[unsaved_sweeps].numpy()))
                prev_veh_pose = np.array(prev_frame.pose.transform, dtype=np.float32).reshape(4, 4)
                tm = np.dot(global_to_curr_veh_frame, prev_veh_pose)
                sweep = {
                    'lidar_path': sequence_name,
                    'sample_idx': cnt,
                    'transform_matrix': tm,
                    'time_lag': 1e-6 * (curr_time_micro - prev_frame.timestamp_micros)
                }
                sweeps.append(sweep)
        info['sweeps'] = sweeps
        densification_infos.append(info)

    return densification_infos
  
def extract_name_time(sequence_file, sampled_interval):
    sequence_name = os.path.splitext(os.path.basename(sequence_file))[0]
